...
export class MineHud {
  ...
  update(snapshot: HudSnapshot) {
    ...
    const energyWidth = Math.max(12, Math.round(220 * normalizedEnergy));
    const energyColor =
    ...
    const comboWidth = Math.max(8, Math.round(72 * Phaser.Math.Clamp(snapshot.comboWindowRatio, 0, 1)));
